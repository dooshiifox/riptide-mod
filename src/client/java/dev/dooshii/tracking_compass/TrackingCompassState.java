...
public class TrackingCompassState extends NeedleAngleState {
    public static final MapCodec<TrackingCompassState> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance
                    .group(
                            Codec.BOOL.optionalFieldOf("wobble", Boolean.TRUE).forGetter(TrackingCompassState::hasWobble))
                    .apply(instance, TrackingCompassState::new));
    private final NeedleAngleState.Angler aimedAngler;
    private final NeedleAngleState.Angler aimlessAngler;
    private final Random random = Random.create();

    public TrackingCompassState(boolean wobble) {
        super(wobble);
    ...
    protected float getAngle(ItemStack stack, ClientWorld world, int seed, Entity user) {
        var component = stack.get(ModComponents.TRACKING_COMPASS_COMPONENT);
        ...
        @Nullable GlobalPos target = component.isTargetedPlayer(user) ? null : component.target().orElse(null);

        return !canPointTo(user, target) ? this.getAimlessAngle(seed, time)
                : this.getAngleTo(user, time, target.pos());
    }

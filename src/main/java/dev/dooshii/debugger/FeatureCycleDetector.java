...
public final class FeatureCycleDetector {
    ...
    public static List<PlacedFeatureIndexer.IndexedFeatures> buildFeaturesPerStep(
        ...
        for (RegistryEntry<Biome> biomeHolder : allBiomes) {
            ...
            final List<RegistryEntryList<PlacedFeature>> features = biomeFeatures.apply(biomeHolder);

            maxSteps = Math.max(maxSteps, features.size());

            for (int stepIndex = 0; stepIndex < features.size(); ++stepIndex) {
                int biomeIndex = 0;
                for (RegistryEntry<PlacedFeature> featureHolder : features.get(stepIndex)) {
